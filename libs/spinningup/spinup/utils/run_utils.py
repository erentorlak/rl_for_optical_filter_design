...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, 
    ...
    # pass thunk through temp file to avoid command-line length issues
    with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".pkl") as f:
        f.write(encoded_thunk)
        temp_fname = f.name

    cmd = [sys.executable if sys.executable else 'python', entrypoint, temp_fname]
    try:
